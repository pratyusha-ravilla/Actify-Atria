...
const browser = await puppeteer.launch({
  headless: true,
  executablePath: chromePath, // ✅ updated
...
const pdf = await page.pdf({
  ...
  margin: {
   ...
   // ✅ Send response
// 1. Generate filename
const fileName = `${(a.activityName || "report")
  .replace(/[^a-z0-9_\-\.]/gi, "_")}_${Date.now()}.pdf`;

// 2. Save PDF in uploads folder
const filePath = path.join(UPLOADS_DIR, fileName);
fs.writeFileSync(filePath, pdf);

// 3. Send URL instead of file
const fileUrl = `http://172.16.38.160:5002/uploads/${fileName}`;
